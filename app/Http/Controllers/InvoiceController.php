...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->invoice_id_test == '') {
                $invoice_init_num = preg_replace('/[^0-9]/', '', $request->invoice_id) + 1;
                $invoice_init_str = preg_replace('/[^a-zA-Z]/', '', $request->invoice_id);
                $this->setting->update(['invoice_initial_value' => $invoice_init_str.$invoice_init_num]);
            }
    ...
    {
        // return $invoice;
        // return view('pdf.index', compact('invoice'), ['invoice' => $invoice->load(['customer', 'invoiceDetails', 'invoiceDetails.product']), 'setting' => $this->setting]);
